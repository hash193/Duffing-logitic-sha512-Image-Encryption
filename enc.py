...
def inverse_permute(flat_img, indices):
    recovered = np.zeros_like(flat_img)
    recovered[indices] = flat_img
    return recovered
...
def plot_histograms(original, encrypted, decrypted):
    ...
    plt.subplot(1, 3, 1)
    plt.hist(original.flatten(), bins=256, range=(0, 255), color='blue', alpha=0.7)
    plt.title("Original Image Histogram")
    plt.xlabel("Pixel Value"); plt.ylabel("Frequency")
    plt.subplot(1, 3, 2)
    plt.hist(encrypted.flatten(), bins=256, range=(0, 255), color='red', alpha=0.7)
    plt.title("Encrypted Image Histogram")
    plt.xlabel("Pixel Value")
    plt.subplot(1, 3, 3)
    plt.hist(decrypted.flatten(), bins=256, range=(0, 255), color='green', alpha=0.7)
    plt.title("Decrypted Image Histogram")
    plt.xlabel("Pixel Value")
    plt.tight_layout()
    plt.show()

def show_images_together(original, encrypted, decrypted):
...
def main(image_path):
    ...
    ssim_enc = ssim(img, encrypted_img)
